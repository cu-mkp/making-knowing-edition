...
function writeEnvFile() {
    const { buildID, editionDataURL, googleTrackingID, releaseMode, website } = configData;
    const { contentEnabled, essaysEnabled, manuscriptEnabled, searchEnabled } = website;
...
REACT_APP_CONTENT_ENABLED=${contentEnabled}
REACT_APP_ESSAYS_ENABLED=${essaysEnabled}
REACT_APP_MANUSCRIPT_ENABLED=${manuscriptEnabled}
REACT_APP_SEARCH_ENABLED=${searchEnabled}
`
...
async function run(mode) {
    switch( mode ) {
        case 'download':
            if (configData?.website && !configData.website.essaysEnabled) {
                console.log("Cannot download essay content when essaysEnabled is false");
            } else {
        ...
        case 'download-thumbs':
            if (configData?.website && !configData.website.essaysEnabled) {
                console.log("Cannot download essay thumbnails when essaysEnabled is false");
            } else {
        ...
        case 'process': 
            if (configData?.website && !configData.website.essaysEnabled) {
                console.log("Cannot process essays when essaysEnabled is false");
            } else {
        ...
        case 'index':
            if (configData?.website) {
                if (!configData.website.essaysEnabled) {
                    console.log("Cannot index essays when essaysEnabled is false");
                } else if (!configData.website.searchEnabled) {
                    console.log("Cannot index when searchEnabled is false")
                }
            } else {
        ...
        case 'manifest':
            if (configData?.website && !configData.website.manuscriptEnabled) {
                console.log("Cannot generate IIIF manifest when manuscriptEnabled is false")
            } else {
        ...
        case 'migrate':
            if (configData?.website && !configData.website.essaysEnabled) {
                console.log("Cannot migrate annotations when essaysEnabled is false")
            } else {
                const annotationMetadata = await loadAnnotationMetadata()
...
function loadConfig(targetName) {
    ...
    // set up config data website
    if (!configData.website) {
        // if website config section is undefined, set all website configs to true
        configData.website = { contentEnabled: true, essaysEnabled: true, manuscriptEnabled: true, searchEnabled: true };
    }
}
