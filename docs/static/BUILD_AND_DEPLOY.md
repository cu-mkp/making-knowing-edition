...
```sh
aws s3 cp --profile mk-aws --recursive static-build/ s3://edition640-dist/<buildID>/static/
```

The mirror then appears at `https://<domain>/static/`. **One-time CloudFront
requirement:** directory URLs (`/static/folios/3r/`) need `index.html`
resolution — attach a viewer-request CloudFront Function to the distribution:

```js
function handler(event) {
    var request = event.request;
    if (request.uri.startsWith('/static/') && request.uri.endsWith('/')) {
...
(`https://edition640.makingandknowing.org/static/sitemap.xml`); it ships with
the next SPA build.
...
- Deep two-pane SPA URLs (`/folios/3r/f/12v/tl`) have no 1:1 static file; the
  ...
  (`/static/folios/3r/#f/tl`). Links inside mirrored content are rewritten
  accordingly (glossary pane URLs go to `/static/glossary/`).
