...
class HashParser extends React.Component {
	...
	setStateWithPath(path) {
			...
			if(left_currentFolioShortID !== '-1'){
				newState.left.folioID=(DocumentHelper.folioURL(left_currentFolioShortID));
				newState.left.folioShortID=left_currentFolioShortID;

				// If locked mode, set right to match
				if(mode === 'l'){
					newState.right.folioID=(DocumentHelper.folioURL(left_currentFolioShortID));
			...
			if(right_currentFolioShortID !== '-1' && mode !== 'l'){
				newState.right.folioID=(DocumentHelper.folioURL(right_currentFolioShortID));
